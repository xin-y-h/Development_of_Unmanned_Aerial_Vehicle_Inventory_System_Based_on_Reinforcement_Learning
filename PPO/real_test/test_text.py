...
inputfile='/home/rvl122-4090/Desktop/hsinying/real_dataset/'
...
for j in range(10):
    ...
    for timestep in range(EP_LEN): 
        ...
        next_state, reward, done,action_to_angle = train_env.env_step(state[0],state[1],state[2],action,state[3]) 
        total_rewards += reward

        img=cv2.imread(inputfile+str(state[0])+"_"+str(state[1])+"_"+str(state[2])+'_Color.png')
        ...
        if done:
            img=cv2.imread(inputfile+str(next_state[0])+"_"+str(next_state[1])+"_"+str(next_state[2])+'_Color.png')
