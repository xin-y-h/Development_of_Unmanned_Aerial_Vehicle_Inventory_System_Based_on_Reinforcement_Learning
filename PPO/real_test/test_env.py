...
class env():
    ...
    def reset(self): 
        ...
        if self.scenes==0: 
            ...
            self.inputfile='/home/rvl122-4090/Desktop/hsinying/real_dataset/'#inputfile
    ...
    def img_resize(self,x,y,angle):
        ...
        img=cv2.imread(self.inputfile+str(x)+"_"+str(y)+"_"+str(angle)+'_Color.png')
        print(np.shape(img))
        print(x,y,angle)
...
# 單張畫面佔比的計算
    ...
    def k_mean_center(self,x,y,angle):
        img=cv2.imread(self.inputfile+str(x)+"_"+str(y)+"_"+str(angle)+'_Color.png')
        model = yolov5.load('./exp8/weights/last.pt')
