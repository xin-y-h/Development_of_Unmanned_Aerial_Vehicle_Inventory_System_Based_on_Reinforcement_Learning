...
time = datetime.now()
path = f'runs/{time.strftime("%Y_%m_%d_%H_%M_%S"+"ppo_ok_choose_action")}'
...
class Actor_Net():
    ...
    def choose_action(self, obs,action_to_angle):
        ...
        if (not action_to_angle):
            if (action==0 or action==1 or action==2 or action==3 or action==5 or action==6 or action==7):
                pass
            else:
            ...
            elif(obs[1]==9 and (action==1 or action==4 or action==5)):
                if( obs[0]==35):
                    if move_change==0:
                        action=torch.tensor([2])
                    if move_change==1:
            ...
            elif(obs[1]==7 and (action==3 or action==6 or action==7)):
                if( obs[0]==35):
                    if move_change==0:
                        action=torch.tensor([1])
                    if move_change==1:
