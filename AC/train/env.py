...
inputfile='./getpicture/shelf_2_change/'#train file path
...
class env():
    ...
    def img_resize(self,x,y,angle):
        ...
        img=cv2.imread(inputfile+str(x)+"_"+str(y)+"_"+str(angle)+'.jpg')
        print(np.shape(img))
        print(x,y,angle)
...
# 單張畫面佔比的計算
    ...
    def k_mean_center(self,x,y,angle):
        img=cv2.imread(inputfile+str(x)+"_"+str(y)+"_"+str(angle)+'.jpg')
        model = yolov5.load('./exp7/weights/last.pt')#yolo weight path
    ...
    def next_state(self,x,y,angle,action_num,done):
        # print("x,y,angle,action_num",x,y,angle,action_num)
