...
class Actor_Net():
    ...
    def choose_action(self, obs,action_to_angle):    
        ...
        if (not action_to_angle):
            ...
            if(obs[0]==12 and (action==0 or action==4 or action==6)):
            ...
            elif(obs[1]==1 and (action==1 or action==4 or action==5)):
                if( obs[0]==12):
                    if move_change==0:
                        action=torch.tensor([2])
                    if move_change==1:
            ...
            elif(obs[1]==-1 and (action==3 or action==6 or action==7)):
                if( obs[0]==12):
                    if move_change==0:
                        action=torch.tensor([1])
                    if move_change==1:
...
def main():
    ...
    for i_episode in range(n_episodes):   
        obs=train_env.reset()# reset environment to initial state 
        action_to_angle=obs[5]
        reward_totle = 0 #remember reward
        ...
        if i_episode % 1 == 0 and i_episode >= 20:
            ...
            # save_data2 = {'net': ResNet.state_dict(), 'i': i_episode}
            if(i_episode==20 or all_ep_r[i_episode]>all_ep_r[i_episode-20]):
            # if(i_episode==20 or all_reward_totle[i_episode]>all_reward_totle[i_episode-20]):
                torch.save(save_data1, "./weight/AC_model_actor.pth")
                # torch.save(save_data2, "./weight/PPO2_model_ResNet.pth")
